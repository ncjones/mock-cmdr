...
describe('session', () => {
  ...
  test('error accessing unknown stub command', () => {
    const session = new Session();
    expect(() => session.stub.myCmd).toThrow('Unknown command: myCmd');
  });

  test('error accessing unknown find command', () => {
    const session = new Session();
    expect(() => session.find.myCmd).toThrow('Unknown command: myCmd');
  });

  test('error accessing unknown nested command', () => {
    const session = new Session();
    cmdRegistry.defineCmd({ type: 'find', path: [ 'foo', 'bar' ], template: () => null });
    expect(() => session.find.foo.myCmd).toThrow('Unknown command: myCmd');
  });
